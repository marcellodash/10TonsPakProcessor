...
namespace BrokenEvent.Shared.WinApi
{
 ...
 class ExplorerIcons
  {
    ...
    public static Icon GetFolderIcon(IconSize size, FolderType folderType)
    {
      ...
      // hack: in 8.1 returns system drive icon if null, so passing any existing dir path
      SHGetFileInfo(Environment.GetFolderPath(Environment.SpecialFolder.ProgramFiles), FILE_ATTRIBUTE_DIRECTORY, ref shfi, (uint)Marshal.SizeOf(shfi), flags);
    ...
    private int folderOpenedIndex = -1;
    private int folderClosedIndex = -1;
    ...
    public ExplorerIcons(ImageList imageList, IconSize iconSize)
    {
    ...
    {
      int result;
      if (cache.TryGetValue(ext, out result))
        return result;

      Icon icon = GetFileIcon(ext, iconSize, false);
      imageList.Images.Add(icon);
      result = imageList.Images.Count - 1; // not thread safe
      cache.Add(ext, result);
      return result;
    }

    public int GetOpenedFolderIconIndex()
    {
      if (folderOpenedIndex != -1)
        return folderOpenedIndex;

      Icon icon = GetFolderIcon(iconSize, FolderType.Open);
      imageList.Images.Add(icon);
      folderOpenedIndex = imageList.Images.Count - 1; // not thread safe
      return folderOpenedIndex;
    }

    public int GetClosedFolderIconIndex()
    {
      if (folderClosedIndex != -1)
        return folderClosedIndex;

      Icon icon = GetFolderIcon(iconSize, FolderType.Closed);
      imageList.Images.Add(icon);
      folderClosedIndex = imageList.Images.Count - 1; // not thread safe
      return folderClosedIndex;
    }
  }
}
